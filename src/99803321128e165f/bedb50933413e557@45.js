// https://observablehq.com/@jeremiak/download-data-button@45
export default function define(runtime, observer) {
  const main = runtime.module();
  main.variable(observer()).define(["DOM","button","data","md"], function(DOM,button,data,md)
{
  const div = DOM.element('div');
  const csvBtn = button(data, 'fips.csv');
  const jsonBtn = button(data, 'fips.json');
  const csvText = md`# Download data button
